...
> `japan_PP-OCRv4_rec_mobile.onnx`（入力高さ48, ModelScope配信）に差し替えています。
> 旧 `japan_rec_crnn_v2.onnx`（高さ32）比で漢字・英数・記号混在の誤りを大きく低減。
> あわせて小さなキャプチャは認識前に自動拡大（`ocr.preprocess`）して精度を底上げします。
